...
var _ = Describe("In", func() {
	...
	BeforeEach(func() {
		pathToIn, err = Build("github.com/jchesterpivotal/knative-service-resource/cmd/in")
	...
	Describe("fetching Service and Revision information is successful", func() {
		JustBeforeEach(func() {
	...
	Context("Something goes wrong while getting the Service and Revision", func() {
		...
		BeforeEach(func() {
			...
			server.RouteToHandler("GET",
				"/apis/serving.knative.dev/v1alpha1/namespaces/default/revisions/test_name",
				ghttp.RespondWithJSONEncoded(http.StatusServiceUnavailable, struct{}{}),
			)
			server.RouteToHandler("GET",
				"/apis/serving.knative.dev/v1alpha1/namespaces/default/services/test_name",
				ghttp.RespondWithJSONEncoded(http.StatusForbidden, struct{}{}),
			)
		})
		...
		It("Prints the error to stderr", func() {
			Eventually(session.Err).Should(Say("failed to get information from Knative: could not get Knative service 'test_name' from Kubernetes:"))
		})
